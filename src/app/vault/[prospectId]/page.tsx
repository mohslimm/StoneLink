...
export default function VaultPage() {
  ...
  const { getProspectById, updateProspect, aiAssets, pushNotification, addTerminalEvent } = useStoneStore();
  
  const prospect = getProspectById(prospectId);
  const assets = prospect ? aiAssets[prospect.id] : null;
  ...
  const handleSign = () => {
    ...
    setTimeout(() => {
      ...
      pushNotification({
        type: 'success',
        title: 'Contrat Signé',
        message: `${prospect.companyName} a signé le contrat via le Vault.`,
      });

      addTerminalEvent({
        type: 'success',
        module: 'vault',
        message: `Signature du contrat confirmée. Vault sécurisé pour ${prospect.companyName}.`,
        prospectId: prospect.id
      });

      // Mocker PDF Generation
